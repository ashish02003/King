 ...
 */
const MugWrapPreview = ({
  photoUrl,
  templateBgUrl,
  wrapType = "mug",
  mockupViews = [],
  slotAssets = {},
  placeholderShapeBySide = {},
  printSizeMm = null,
}) => {
  ...
  const hasSameBackgrounds =
    effectiveMockupViews.length > 0 &&
    effectiveMockupViews.every(
      (m) => m.backgroundUrl === effectiveMockupViews[0].backgroundUrl,
    );
  ...
  const ALL_VIEWS =
    effectiveMockupViews.length > 0
      ? effectiveMockupViews.map((mv) => ({
          ...
          type: "photographic",
          offsetRatio:
            mv.side === "left" ? 0.25 : mv.side === "right" ? 0.75 : 0.5,
          flipBg:
            mv.side === "right" &&
            (wrapType === "mug" ||
              wrapType === "bottle" ||
  ...
  const printRatio =
    printSizeMm && printSizeMm.widthMm && printSizeMm.heightMm
  ...
  const canvasHeight = isPhotographicView
    ? 650
    : Math.round((PREVIEW_CANVAS_W / printRatio) * 1.3);
  ...
  useEffect(() => {
    ...
    const drawBackgroundWithTransform = (
      img,
      boxW,
      boxH,
      bgTransform = null,
      ...
      const dh = ih * scale;
      const rawDx =
        (boxW - dw) / 2 + (Number(bgTransform?.offsetX) || 0) * boxW * 0.24;
      const rawDy =
        (boxH - dh) / 2 + (Number(bgTransform?.offsetY) || 0) * boxH * 0.24;
      ...
      const isWrapProduct =
        wrapType === "mug" || wrapType === "bottle" || wrapType === "planter";
      const hideHandleTrigger =
        activeView.side === "center" && isWrapProduct && hasSameBackgrounds;
      ...
      if (activeView.type === "photographic") {
        ...
        bgImg.onload = () => {
          ...
          applyShapeClip(
            ...
            destW,
            destH,
            activeView.customPath,
            placementCurve,
          );

          const isWrapShape = shapeType === "mug-wrap" || shapeType === "wave";
          const effectiveCurve = isWrapShape
            ? placementCurve
            : Number(p.curve) || 0;
          ...
          if (effectiveCurve > 0 && designImg && isWrapShape) {
            ...
            const viewCenter =
              designImg.width * (activeView.offsetRatio || 0.5);
            ...
            if (startX + viewWidth > designImg.width)
              startX = designImg.width - viewWidth;
          ...
          if (selectedText) {
            ...
            lines.forEach((l, i) =>
              ctx.fillText(l, destW / 2, startY + i * lineH),
            );
          }
        ...
        if (designImg) {
          ...
          const { dx, dy, dw, dh } = getDrawBox(
            { width: viewWidth, height: ih },
            slotW,
            slotH,
            selectedTransform,
          );
        ...
        if (selectedText) {
          const textClipKind = normalizeShapeType(
            slotShapeType || placeholderShapeBySide[activeSlotKey] || "rounded",
          );
          ctx.save();
          ...
          lines.forEach((l, i) =>
            ctx.fillText(l, slotX + slotW / 2, startY + i * lineH),
          );
          ctx.restore();
        }
        ...
        ctx.save();
        const mugBodyGrad = ctx.createLinearGradient(
          mugX,
          0,
          mugX + mugWidth,
          0,
  ...
  const handlePrev = () =>
    setCurrentViewIndex((p) => (p === 0 ? ALL_VIEWS.length - 1 : p - 1));
  const handleNext = () =>
    setCurrentViewIndex((p) => (p === ALL_VIEWS.length - 1 ? 0 : p + 1));
  ...
  return (
    <div className="w-full flex flex-col items-center animate-fade-in-up bg-white rounded-[2rem] border border-gray-100 p-6 relative shadow-sm">
      <div className="relative flex items-center justify-center w-full">
        <button
          onClick={handlePrev}
          className="absolute z-10 flex items-center justify-center w-10 h-10 text-gray-400 transition-all bg-white border border-gray-200 rounded-full shadow-md left-2 hover:text-indigo-600 hover:scale-110"
        >
          <FaChevronLeft />
        </button>
        ...
        <button
          onClick={handleNext}
          className="absolute z-10 flex items-center justify-center w-10 h-10 text-gray-400 transition-all bg-white border border-gray-200 rounded-full shadow-md right-2 hover:text-indigo-600 hover:scale-110"
        >
          <FaChevronRight />
        </button>
      </div>
      <div className="flex gap-2 mt-4">
        {ALL_VIEWS.map((v, i) => (
          <button
            ...
            className={`w-2 h-2 rounded-full ${i === currentViewIndex ? "bg-indigo-600 w-4" : "bg-gray-200"} transition-all`}
          />
        ))}
      </div>
