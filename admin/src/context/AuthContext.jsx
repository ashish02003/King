...
export const AuthProvider = ({ children }) => {
  ...
  const updateAvatar = async (file, onProgress) => {
    try {
      ...
      const { data } = await axios.put(`${API_BASE}/auth/avatar`, formData, {
        headers: {
  ...
  const deleteAvatar = async () => {
    try {
      const { data } = await axios.delete(`${API_BASE}/auth/avatar`, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
      const updatedUser = { ...data, token: user.token };
      persistUser(updatedUser);
  ...
  return (
    <AuthContext.Provider
      value={{ user, login, register, logout, updateProfile, updateAvatar, deleteAvatar, loading }}
    >
      {children}
    </AuthContext.Provider>
  );
